...
class MyMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ui.late_spin_box.valueChanged.connect(self.update_total_price)
        self.ui.cio_spin_box.valueChanged.connect(self.update_total_price)
        ...
        self.kiosk_client.connect(("127.0.0.1", 20000))  # 서버 IP 주소와 포트 번호를 지정
    ...
    def on_button_clicked(self):
        ...
        num_late = self.ui.late_spin_box.value()
        if num_late > 0:
            # 처리
            index = 2
            ea = num_late
        ...
        num_cio = self.ui.cio_spin_box.value()
        if num_cio > 0:
            # 처리
            index = 3
            ea = num_cio
        ...
        self.ui.late_spin_box.setValue(0)
        self.ui.cio_spin_box.setValue(0)
        self.ui.morning_spin_box.setValue(0)
    ...
    def update_total_price(self):
        ...
        late_value = self.ui.late_spin_box.value()
        late_price = self.ui.late_price.text()[:4]
        late_reslut = late_value * int(late_price)
        total += late_reslut

        cio_value = self.ui.cio_spin_box.value()
        cio_price = self.ui.cio_price.text()[:4]
        cio_reslut = cio_value * int(cio_price)
        total += cio_reslut
