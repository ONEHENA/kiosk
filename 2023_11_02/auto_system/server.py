...
def kiosk_task(order_num, client_socket):
    while True:
        ...
        client_socket.send(f'주문번호 {order_num}번 완료되었습니다.'.encode('utf-8'))
        order_num += 1
...
def coffee_task(client_socket, order_num):
    while True:
        ...
        if message:
            ...
            if order_dict[order_num] > 0:
                order_dict[order_num] -= 1
            else:
                # 전광판에 완료 신호를 보낸다.
                clients['view'].send(message.encode('utf-8'))    
...
while True:
    ...
    threading.Thread(target=handler, args=(client_socket, order_num
                                           , name,)).start()